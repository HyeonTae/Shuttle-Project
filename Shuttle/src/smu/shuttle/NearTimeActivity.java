...
public class NearTimeActivity extends Activity {
	...
	// final String serverIp = "http://192.168.0.7:8060";
	...
	protected void showListView() {
		try {
			...
			ListAdapter adapter = new SimpleAdapter(NearTimeActivity.this, timeBusList, R.layout.list_item,
			...
			try {
				if (location == "아산시") {
					...
					conn = (HttpURLConnection) new URL(
							params[0] + "/ShuttleServer/main.do?" + "action=searchAllBusForClass").openConnection();
					...
					// map = mapper.readValue(sb.toString(), new
					// TypeReference<HashMap<String, String>>() {});
					// System.out.println("fdsfdsf"+map);
					return sb.toString().trim();

				} else if (location == "천안시") {
					conn = (HttpURLConnection) new URL(
							params[0] + "/ShutlleServer/main.do?" + "action=searchBusFromCheonAnToSMU")
									.openConnection();
				} else if (location == "동남구") {
					conn = (HttpURLConnection) new URL(
							params[0] + "/ShutlleServer/main.do?" + "action=searchBusFromTerminalToSMU")
									.openConnection();
				} else {
					conn = (HttpURLConnection) new URL(
							params[0] + "/ShutlleServer/main.do?" + "action=searchBusForAsan").openConnection();
				}
